...
void test_move_assignment_noexcept() {
  ...
  {
    using V = swl::variant<MoveOnlyNT>;
    static_assert(!std::is_nothrow_move_assignable<V>::value, "");
  }
  {
    using V = swl::variant<MoveOnlyOddNothrow>;
    static_assert(!std::is_nothrow_move_assignable<V>::value, "");
  }
}
...
void test_move_assignment_sfinae() {
  ...
  {
    using V = swl::variant<int, CopyOnly>;
    static_assert(std::is_move_assignable<V>::value, "");
  }
  {
  ...
  }
  {
    ...
    // a move constructor.
    using V = swl::variant<int, MoveAssignOnly>;
    static_assert(!std::is_move_assignable<V>::value, "");
  }
  ...
  {
    using V = swl::variant<int, NTMoveAssign>;
    static_assert(!std::is_trivially_move_assignable<V>::value, "");
    static_assert(std::is_move_assignable<V>::value, "");
  }
  {
    using V = swl::variant<int, TMoveAssign>;
    static_assert(std::is_trivially_move_assignable<V>::value, "");
  }
  {
    using V = swl::variant<int, TMoveAssignNTCopyAssign>;
    static_assert(std::is_trivially_move_assignable<V>::value, "");
  }
  {
    using V = swl::variant<int, TrivialCopyNontrivialMove>;
    static_assert(!std::is_trivially_move_assignable<V>::value, "");
  }
  {
    using V = swl::variant<int, CopyOnly>;
...
void test_move_assignment_non_empty_empty() {
  ...
  }
  {
    ...
    V v1(swl::in_place_index<2>, "hello");
    V v2(swl::in_place_index<0>);
    makeEmpty(v2);
    V &vref = (v1 = std::move(v2));
    SWL_ASSERT(&vref == &v1);
...
void test_move_assignment_same_index() {
  ...
  {
    using V = swl::variant<int, MoveAssign, unsigned>;
    V v1(swl::in_place_type<MoveAssign>, 43);
    V v2(swl::in_place_type<MoveAssign>, 42);
    MoveAssign::reset();
    V &vref = (v1 = std::move(v2));
    SWL_ASSERT(&vref == &v1);
    SWL_ASSERT(v1.index() == 1);
    SWL_ASSERT(swl::get<1>(v1).value == 42);
    SWL_ASSERT(MoveAssign::move_construct == 0);
    SWL_ASSERT(MoveAssign::move_assign == 1);
  }
#ifndef TEST_HAS_NO_EXCEPTIONS
  using MET = MakeEmptyT;
  {
    using V = swl::variant<int, MET, std::string>;
    V v1(swl::in_place_type<MET>);
    MET &mref = swl::get<1>(v1);
  ...
  }
  {
    struct {
      constexpr Result<int> operator()() const {
        using V = swl::variant<int, TMoveAssign, unsigned>;
        V v(swl::in_place_type<TMoveAssign>, 43);
        V v2(swl::in_place_type<TMoveAssign>, 42);
        v = std::move(v2);
        return {v.index(), swl::get<1>(v).value};
      }
    } test;
    constexpr auto result = test();
    static_assert(result.index == 1, "");
...
void test_move_assignment_different_index() {
  ...
  }
  {
    ...
    SWL_ASSERT(v1.index() == 1);
    SWL_ASSERT(swl::get<1>(v1).value == 42);
    SWL_ASSERT(MoveAssign::move_construct == 1);
    SWL_ASSERT(MoveAssign::move_assign == 0);
  }
#ifndef TEST_HAS_NO_EXCEPTIONS
  using MET = MakeEmptyT;
  {
    using V = swl::variant<int, MET, std::string>;
    V v1(swl::in_place_type<int>);
    V v2(swl::in_place_type<MET>);
    try {
      v1 = std::move(v2);
      SWL_ASSERT(false);
  ...
  }
  {
    struct {
      constexpr Result<long> operator()() const {
        using V = swl::variant<int, TMoveAssign, unsigned>;
        V v(swl::in_place_type<unsigned>, 43u);
        V v2(swl::in_place_type<TMoveAssign>, 42);
        v = std::move(v2);
        return {v.index(), swl::get<1>(v).value};
      }
...
constexpr bool test_constexpr_assign_imp(
    swl::variant<long, void*, int>&& v, ValueType&& new_value)
{
  swl::variant<long, void*, int> v2(
      std::forward<ValueType>(new_value));
  const auto cp = v2;
  v = std::move(v2);
  return v.index() == NewIdx &&
        swl::get<NewIdx>(v) == swl::get<NewIdx>(cp);
}

void test_constexpr_move_assignment() {
  // Make sure we properly propagate triviality, which implies constexpr-ness (see P0602R4).
#if TEST_STD_VER > 17
  using V = swl::variant<long, void*, int>;
